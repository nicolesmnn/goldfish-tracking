...
if (!localStorage.getItem("trackingUserId")) {
  const randomId =
    "usr_" + Math.random().toString(36).substring(2, 5).toUpperCase();
...
const facts = [
  {
    num: "01",
    title: "8 SEKUNDEN",
    text: "Der Goldfisch-Mythos: Angeblich ist unsere Aufmerksamkeitsspanne auf 8 Sekunden gesunken. Kürzer als die eines Goldfisches.",
  },
  {
    num: "02",
    title: "DOOM SCROLLING",
    text: "Warum du nicht aufhören kannst? Unendliche Feeds manipulieren dein Dopaminsystem wie ein Spielautomat.",
  },
  {
    num: "03",
    title: "MULTI-TASKING LÜGE",
    text: "Task-Switching kostet das Gehirn enorm viel Energie. Wer ständig wechselt, verliert bis zu 40% seiner Produktivität.",
  },
  {
    num: "04",
    title: "PHANTOM VIBRATION",
    text: "Bis zu 80% der Menschen spüren ihr Handy vibrieren, obwohl gar nichts passiert. Unser Gehirn ist hyper-alarmiert.",
  },
  {
    num: "05",
    title: "174 ZEITUNGEN",
    text: "Wir konsumieren heute täglich so viele Daten, wie in 174 dicke Zeitungen passen würden. Kein Wunder, dass das Gehirn oft streikt.",
  },
  {
    num: "06",
    title: "23 MINUTEN",
    text: "So lange braucht dein Gehirn, um nach einer einzigen Unterbrechung (wie einer Push-Nachricht) wieder volle Konzentration aufzubauen.",
  },
  {
    num: "07",
    title: "TIKTOK BRAIN",
    text: "Endlose Kurzvideos programmieren das Gehirn auf sofortige Belohnung um. Langsame, tiefe Gedankengänge werden physisch anstrengender.",
  },
  {
    num: "08",
    title: "DIGITAL AMNESIA",
    text: "Das Gehirn lagert Erinnerungen ans Smartphone aus. Was wir sofort ergoogeln können, speichern wir biologisch nicht mehr ab.",
  },
  {
    num: "09",
    title: "F-PATTERN",
    text: "Wir lesen Texte am Bildschirm nicht mehr, wir scannen sie nur noch grob in einer F-Form. Tiefes Textverständnis geht massiv verloren.",
  },
  {
    num: "10",
    title: "BLAULICHT KATER",
    text: "Bildschirme vor dem Schlafen killen die Melatonin-Produktion. Der fehlende Tiefschlaf zerstört die Konzentration für den gesamten nächsten Tag.",
  },
];
...
if (typeof gsap !== "undefined") {
  if (
    ...
    document.querySelector(".consent-box")
  ) {
...
if (typeof gsap !== "undefined" && attentionTest && fakeNotif) {
  ...
  ScrollTrigger.create({
    ...
    onLeave: () =>
      gsap.to(fakeNotif, { opacity: 0, scale: 0.8, duration: 0.2 }),
    onEnterBack: () =>
      gsap.to(fakeNotif, { opacity: 1, scale: 1, duration: 0.2 }),
    onLeaveBack: () =>
      gsap.to(fakeNotif, { opacity: 0, scale: 0.8, duration: 0.2 }),
  ...
  fakeNotif.addEventListener("click", () => {
    ...
    fakeNotif.innerHTML =
      '<div class="notif-text"><strong>REINGEFALLEN.</strong><br>Aufmerksamkeit = 0.</div>';
...
function checkAnswer(isCorrect, btnElement) {
  ...
  if (resultMsg) {
    ...
    if (isCorrect) {
      ...
      resultMsg.innerHTML =
        "EXAKT.<br>Du hast bewiesen: Deine Aufmerksamkeitsspanne existiert nicht.";
    } else {
      resultMsg.style.color = "red";
      resultMsg.innerHTML =
        "FALSCH.<br>Du hast das Gedächtnis eines Goldfisches.";
    }
  }
}
...
const dopamineFeed = [
  {
    text: "Ein virales Katzenvideo 🐱",
    visual: "<span class='mill-emoji'>🐱 SCREENSHOT 🐱</span>",
  },
  {
    text: "Jemand is im Internet wütend 😡",
    visual: "<span class='mill-emoji'>🤬 ALERT 🤬</span>",
  },
  {
    text: "Ein tanzender Teenie auf TikTok 🕺",
    visual: "<span class='mill-emoji'>🔥 TRENDING 🔥</span>",
  },
  {
    text: "Dieser Klick wird dein Leben verändern ⚡",
    visual: "<span class='mill-emoji'>⚡ 100% DOPAMIN ⚡</span>",
  },
  {
    text: "Werbung für Socken, die du gestern gesucht hast 🧦",
    visual: "<span class='mill-emoji'>💸 BUY NOW 💸</span>",
  },
];
...
if (millBox) {
  millBox.addEventListener("scroll", () => {
    const isAtBottom =
      millBox.scrollHeight - millBox.scrollTop <= millBox.clientHeight + 50;

    if (isAtBottom) {
      const currentItem =
        dopamineFeed[Math.floor(Math.random() * dopamineFeed.length)];
      ...
      if (millBox.children.length === 15) {
        ...
        banner.style =
          "position:fixed; top:50%; left:50%; transform:translate(-50%,-50%); background:red; color:white; font-size:4rem; font-weight:900; padding:20px 40px; z-index:10000; border:10px solid black; text-align:center; box-shadow: 15px 15px 0px #000; font-family: sans-serif;";
...
if (panicBtn) {
  ...
  panicBtn.addEventListener("click", () => {
    if (hasClickedPanic) telemetryData.rageClicks++;
  });
}
...
document
  ...
  .forEach((sec) => trackingObserver.observe(sec));

//12. Datenschutz
function sendTelemetry() {
  if (localStorage.getItem("userConsent") !== "true") return;

  telemetryData.timeSpentOnPage = Math.floor(
...
document.addEventListener("DOMContentLoaded", () => {
  const auswertungBtn = Array.from(document.querySelectorAll("button, a")).find(
    (el) => el.textContent.includes("AUSWERTUNG"),
  );
  if (auswertungBtn) {
...
if (cursor && typeof gsap !== "undefined") {
  ...
  window.addEventListener("mousemove", (e) => {
    ...
    const angle =
      (Math.atan2(e.clientY - pos.y, e.clientX - pos.x) * 180) / Math.PI;
